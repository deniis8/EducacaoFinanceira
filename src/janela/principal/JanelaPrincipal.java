...
public class JanelaPrincipal {
	...
	public void janela() {
		...
		lblSaldo.setBounds(03, 475, 200, 15);
		lblInvF.setBounds(03, 490, 200, 15);
		lblInvV.setBounds(180, 475, 200, 15);
		lblTotalAP.setBounds(490, 475, 140, 15);
		lblTotalP.setBounds(490, 490, 140, 15);
		lblTotalAR.setBounds(660, 475, 140, 15);
		 ...
		 * 
		 * @Override public void actionPerformed(ActionEvent e) { rodarThred(); } });
		 */
		...
		btnPesq.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				consulta.pesqLan(model, tblTab, txtDe, txtAte, checRecebido, checPago, checAPagar, checAReceber,
						lblTotalAP, lblTotalP, lblTotalAR, lblTotalR, boxGCCusto);
			}
		});
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (indiceLinha != -1) {
					...
					if (!txtCod.getText().equals("")) {
						consulta.selectComboboxCentroCusto(boxCCusto, "S");
						consulta.selectLanJ(txtCod.getText(), txtEmissao, txtHora, txtValor, txtDesc, boxStatus,
								boxCCusto);
						dialogP.setVisible(true);
					}
				} else {
	...
	public void janelaInc(String titulo) {
		...
		// Captura a data atual (dia/mÃªs/ano)
		Date dt = new Date();
		...
		btnConfirm.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (opc == "i") {
					if (!txtEmissao.getText().equals("") && !txtHora.getText().equals("")
							&& !txtValor.getText().equals("") && !txtDesc.getText().equals("")) {

						crudLancamento.incluirLancamento(txtEmissao.getText(), txtHora.getText(),
								Double.parseDouble(txtValor.getText()), txtDesc.getText(),
								boxStatus.getSelectedItem().toString(), boxCCusto);
						limpCampos(opc);
						try {
							consulta.selectLanca(model, tblTab, lblTotalAP, lblTotalP, lblTotalAR, lblTotalR);
						} catch (ParseException e) {
					...
					int resposta = JOptionPane.showOptionDialog(null, "Deseja excluir esse Lançamento?", "Pergunta",
	...
	public void janelaCCusto() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (indiceLinha != -1) {
					if (!tblTabCC.getValueAt(indiceLinha, 0).toString().equals("")) {
						janeInserCC("2", tblTabCC.getValueAt(indiceLinha, 0).toString(),
								tblTabCC.getValueAt(indiceLinha, 1).toString());
					}
				} else {
					JOptionPane.showMessageDialog(null, "Selecione um Registro!");
				}
		...
		btnCcExc.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (indiceLinha != -1) {
					if (!tblTabCC.getValueAt(indiceLinha, 0).toString().equals("")) {
						janeInserCC("3", tblTabCC.getValueAt(indiceLinha, 0).toString(),
								tblTabCC.getValueAt(indiceLinha, 1).toString());
					}
				} else {
					JOptionPane.showMessageDialog(null, "Selecione um Registro!");
		...
		btnConf.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (opc == "1") {
					...
					int resposta = JOptionPane.showOptionDialog(null, "Deseja excluir esse registro?", "Pergunta",
							JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
					if (resposta == 0) {
	...
	/*
	 * public void rodarThred() { JDialog dlgProgress = new JDialog();
	 * dlgProgress.setTitle("Calculando..."); JLabel lblStatus = new
	 * JLabel("Calculando..."); JProgressBar progress = new JProgressBar();
	 * dlgProgress.setSize(300, 90); dlgProgress.setLocationRelativeTo(null); //
	 * dlgProgress.setModal(true); dlgProgress.setLayout(null);
	 * lblStatus.setBounds(05, 05, 300, 15); progress.setBounds(05, 25, 270, 15);
	 * dlgProgress.add(lblStatus); dlgProgress.add(progress);
	 * 
	 * String idReg = ""; String data = ""; //int resposta =
	 * JOptionPane.showConfirmDialog(null, "Recalcular Tudo", "Recalcular Tudo",
	 * JOptionPane.NO_OPTION); Object[] options = { "Sim", "NÃ£o", "Cancelar" }; int
	 * resposta = JOptionPane.showOptionDialog(null, "Deseja recalcular tudo?",
	 * "InformaÃ§Ã£o", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE,
	 * null, options, options[0]);
	 * 
	 * if (resposta == 1) { int indiceLinha = tblTab.getSelectedRow(); if
	 * (indiceLinha != -1) { idReg= tblTab.getValueAt(indiceLinha, 0).toString();
	 * data = tblTab.getValueAt(indiceLinha, 1).toString(); } else {
	 * JOptionPane.showMessageDialog(null, "Selecione um Registro!"); } } if
	 * (resposta == 0 || resposta == 1) { dlgProgress.setVisible(true);
	 * RecalculaSaldo rec = new RecalculaSaldo(progress, dlgProgress, lblStatus,
	 * lblSaldo, lblInvF, lblInvV, idReg, data); Thread t = new Thread(rec);
	 * t.start(); } }
	 */
